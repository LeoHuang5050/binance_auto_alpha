                ...
                if data.get('code') == '000000' and 'data' in data:
                    ...
                    if len(orders) > 0:
                        ...
                        for i, order in enumerate(orders, 1):
                            ...
                            print(f"  价格: {order.get('price')} USDT")
                            print(f"  数量: {order.get('origQty')}")
                            print(f"  已成交数量: {order.get('executedQty')}")
                            print(f"  平均成交价: {order.get('avgPrice')} USDT")
                            print(f"  累计成交金额: {order.get('cumQuote')} USDT")
                            print(f"  手续费: {order.get('commission')}")
                            print(f"  基础资产: {order.get('baseAsset')}")
                            print(f"  计价资产: {order.get('quoteAsset')}")
                            print(f"  有效期类型: {order.get('timeInForce')}")
...
def main():
    ...
    print(f"今天0点时间戳: {start_time} ({start_dt.strftime('%Y-%m-%d %H:%M:%S')})")
    print(f"明天0点时间戳: {end_time} ({end_dt.strftime('%Y-%m-%d %H:%M:%S')})")
    print()
    
    result = test_order_history()
